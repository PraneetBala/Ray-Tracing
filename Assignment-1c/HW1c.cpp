...
Date: 10/13/2019
Code: Assignment 1c
Objective: To implement polygons, textures and parallel projection
...
ColorType Trace_Ray(RayType ray, vector<sphere_t> & sphere, vector<ColorType> & mtl, vector<LightType> & light, vector<size3_t> & v_points, vector<FaceType> & face, vector<size3_t> & v_normals, vector<size2_t> & v_tex, vector<TexType> & tex) {
	...
	if (min_T != 100000) {
		...
		inter1[0] = sphere[pa1].x + 1.006*N1[0]*sphere[pa1].r;
		inter1[1] = sphere[pa1].y + 1.006*N1[1]*sphere[pa1].r;
		inter1[2] = sphere[pa1].z + 1.006*N1[2]*sphere[pa1].r;
	}
	float min2 = min_T;
	...
	if (min_T == 100000) {
		...
		if (min_T != min2) {	// When triangle is closer
			...
			if (sphere[pa].tex == 1) {
				...
				//phi = acos(-(inter[2]-sphere[pa].z)/sphere[pa].r);
				//theta = atan2(-(inter[1]-sphere[pa].y),(inter[0]-sphere[pa].x));
...
int main(int argc, char** argv) {
	...
	for (int i = 0; i < height; i++) {
		for (int j = 0; j < width; j++) {
			...
			color = Trace_Ray(ray, sphere, mtl, light, v_points, face, v_normals, v_tex, tex);
			r = int(255*color.odr);
			g = int(255*color.odg);
			b = int(255*color.odb);
